...
public class ChatBackgroundService extends Service {
    ...
    private Runnable myTask = new Runnable() {
        ...
        public void run() {
            //Do all work here
            Log.i("background", "CHAT BACKGROUND SERVICE WORKING");
            getAllBadge();
            stopSelf();
        }
    };
        ...
        tokenRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()){
                    if (dataSnapshot.hasChild("chatDelivery")){
                        String qq = dataSnapshot.child("chatDelivery").getValue().toString();
                        if (qq.equals("no")){
                            if (dataSnapshot.hasChild("chat")){
                                String ww = dataSnapshot.child("chat").getValue().toString();
                                if (ww.equals("no")){
                                    if (dataSnapshot.hasChild("chatMessage")){
                                        tokenRef.child("chatDelivery").setValue("yes");
                                        String message = dataSnapshot.child("chatMessage").getValue().toString();
                                        notifyUser(message);
                                    }
                                }
    ...
    private void notifyUser(String msg) {
        ...
        remoteViews.setImageViewResource(R.id.notificationOrder_backIcon, R.drawable.notify_chat);
